import 'package:beerpong_leaderboard/screens/external_profile/external_profile_head.dart';
import 'package:beerpong_leaderboard/screens/external_profile/external_profile_trophy.dart';
import 'package:beerpong_leaderboard/services/database.dart';
import 'package:beerpong_leaderboard/utilities/constants.dart';
import 'package:beerpong_leaderboard/utilities/trophy.dart';
import 'package:beerpong_leaderboard/utilities/user.dart';
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';

class name extends StatelessWidget {
  const name({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Container();
  }
}

class ExternalProfile extends StatefulWidget {
  final String name;
  const ExternalProfile({required this.name});

  @override
  State<ExternalProfile> createState() => _ExternalProfileState();
}

class _ExternalProfileState extends State<ExternalProfile> {
  Future<void> _pullRefresh() async {}

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFF6CA8F1),
      appBar: AppBar(
        backgroundColor: Colors.blue[320],
        title: Text(
          widget.name,
          style: kHintTextStyle,
        ),
        elevation: 0.0,
      ),
      body: RefreshIndicator(
        onRefresh: _pullRefresh,
        child: Container(
          height: MediaQuery.of(context).size.height * 1.4,
          child: SingleChildScrollView(
            child: Column(
              children: <Widget>[
                FutureBuilder<UserModel>(
                    future: context
                        .read<DatabaseService>()
                        .getSpecificUser(widget.name),
                    builder: (BuildContext context,
                        AsyncSnapshot<UserModel> snapshot) {
                      if (snapshot.hasData) {
                        return ExternalProfileHead(user: snapshot.data!);
                      } else if (snapshot.hasError) {
                        return getLoadingFields([1], 100.0, 100.0,
                            MediaQuery.of(context).size.height * 0.3);
                      } else {
                        return getLoadingFields([1], 100.0, 100.0,
                            MediaQuery.of(context).size.height * 0.3);
                      }
                    }),
                const SizedBox(
                  height: 40.0,
                ),
                FutureBuilder<TrophyModel>(
                    future: context
                        .read<DatabaseService>()
                        .getSpecificUserTrophy(widget.name),
                    builder: (BuildContext context,
                        AsyncSnapshot<TrophyModel> snapshot) {
                      if (snapshot.hasData) {
                        return ExternalTrophyHead(trophy: snapshot.data!);
                      } else if (snapshot.hasError) {
                        return getLoadingFields([1], 100.0, 100.0,
                            MediaQuery.of(context).size.height * 0.3);
                      } else {
                        return getLoadingFields([1], 100.0, 100.0,
                            MediaQuery.of(context).size.height * 0.3);
                      }
                    }),
              ],
            ),
          ),
        ),
      ),
    );
  }
}
